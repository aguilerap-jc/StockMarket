...
def get_stock_quandl(stock='AAPL',db = "WIKI",start=DEFAULT_START_DATE,end=DEFAULT_END_DATE):
    ds = db+"/"+stock
    df_quandl = quandl.get(dataset=ds,start_date=start,end_date=end)
    return df_quandl

def get_stock_intrinio(id="AAPL",start=DEFAULT_START_DATE,end=DEFAULT_END_DATE,freq="daily",page_s=10000):
    r = security_api.get_security_stock_prices(identifier=id,start_date=start,frequency=freq,page_size=page_s)
...
def get_stock_iexfinance(stock="AAPL",start=DEFAULT_START_DATE,end=DEFAULT_END_DATE,output_form="pandas"):
    ...
    df_iex = get_historical_data(stock,start,end,output_format=output_form,token=IEXFINANCE_KEY)
    return df_iex
    
def get_stock_tiingo(stock="AAPL",start=DEFAULT_START_DATE,end=DEFAULT_END_DATE):
    historical_p = tiingo_client.get_dataframe(stock, startDate=start, endDate=end,frequency="daily")
    return historical_p
